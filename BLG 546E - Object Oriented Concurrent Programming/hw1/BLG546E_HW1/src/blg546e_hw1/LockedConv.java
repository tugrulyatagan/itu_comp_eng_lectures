...
public class LockedConv extends Conv {
    ...
    public LockedConv(int[][] input_matrix, int[][] input_kernel) {
        super(input_matrix, input_kernel);
        lock = new ReentrantLock();
    }

    public int inc_and_get_filter_order() {
        int tmp_filter_order_counter = 0;
        lock.lock();
        try {
            filter_order_counter++;
            tmp_filter_order_counter = filter_order_counter;
        } catch(Exception e) {
            e.printStackTrace();
        }
        finally {
        ...
        return tmp_filter_order_counter;
    }
