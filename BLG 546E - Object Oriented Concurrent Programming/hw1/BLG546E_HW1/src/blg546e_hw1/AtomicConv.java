...
public class AtomicConv extends Conv {
    private AtomicInteger filter_order_counter; /* shared atomic variable among threads */

    private int num_of_current_filter;

    public AtomicConv(int[][] input_matrix, int[][] input_kernel) {
        super(input_matrix, input_kernel);
        filter_order_counter = new AtomicInteger();
    }
    ...
    Runnable worker = () -> {
        int my_filter_order = filter_order_counter.incrementAndGet();
