...
bool DirectedGraph::addEdge(string aNode,string bNode){
...
bool DirectedGraph::deleteEdge(string aNode,string bNode){
...
void DirectedGraph::intersect(const DirectedGraph& aGraph){
...
void DirectedGraph::unite(const DirectedGraph& aGraph){
     ...
     for(i=0;i<this->nofNodes;i++){
          indice=(const_cast <DirectedGraph*> (&aGraph))->containsNode(this->nodes[i].getNodeName());
          if(indice>=0)
...
void DirectedGraph::acceptTraverse(Traversal* traverser){

    cout<<"DirectedGraph: "<<traverser->traverse(*this)<<endl;

}

//Query if the directed graph accepts DF traversal	
bool DirectedGraph::acceptsDF(){
    return false;
}
