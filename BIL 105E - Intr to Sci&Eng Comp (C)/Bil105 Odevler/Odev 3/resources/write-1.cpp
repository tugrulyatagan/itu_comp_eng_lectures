...
{	
	out_file << "The result matrix is :" << endl << endl;
	for (int i = 0; i < row; i++)
	{	
		for (int j = 0; j < col; j++)
			out_file << setfill (' ') << right << setw (5) << matrix[i][j];
		out_file << endl;
	}
}
...
void print_matrix(double **matrix, int row, int col)
{
	...
	{
		for (int j = 0; j < col; j++)
			cout << setfill (' ') << right << setw (5) << matrix[i][j];
		cout << endl;
	}
}
...
void print_array(double *array, int row, int col)
{
	...
	for (int i = 0; i < row * col; i++)
		cout << setfill (' ') << right << setw (5) << array[i];
	cout << endl;
}
...
{																  		  //r1    c1=r2   c2
	...
	{
		array_result[a] = 0;
		for (int b = 0; b < m; b++)// eleman sayisi
